...
foreach ($mensajes as $mensaje) {
	...
	';
		...
		$publicacion = ($mensaje->producto->id)?Doctrine::getTable('publicacion')->findOneByid_producto($mensaje->producto->id):'';
	
	
	
	$idPublicacion = ($publicacion)?'Publicaci&oacute;n: '.$publicacion->id:'';
	$html = str_replace('<!--{mensajePublicacion}-->', $idPublicacion, $html);	
