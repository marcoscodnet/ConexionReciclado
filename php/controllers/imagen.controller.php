...
class UploadHandler {
    ...
    function __construct($options = null) {
        $this->options = array(
            ...
            'upload_url' => RUTA . '/images/productos/',
            'param_name' => 'files',
            ...
            'image_versions' => array(
                ...
                'large' => array(
                    ...
                    'upload_url' => RUTA . '/images/productos/gr/',
                    'max_width' => 160,
                    'max_height' => 160
                ),
                'thumbnail' => array(
                    'upload_dir' => RUTA_IMAGENES . 'ch/',
                    'upload_url' => RUTA . '/images/productos/ch/',
                    'max_width' => 70,
                    'max_height' => 70
                )
            )
    ...
    private function determinarNombre($type) {
        ...
        $ext = $extArray[1];
        $id = (isset($_POST['productoId']) && $_POST['productoId'] != 0) ? $_POST['productoId'] : $_SESSION['codigoUsuario'];
    ...
    private function borrarSobrantes() {
        //$id = Producto::ultimoId()+1;
        $id = $_SESSION['codigoUsuario'];
        Doctrine_Query::create()->delete('Imagen')->where('ruta like ?', 'producto'.$_SESSION['codigoUsuario'].'%')->execute();
        for ($i = 1; $i <= 4; $i++) {
            $rutaJpg = 'producto' . $id . '.' . $i . '.jpeg';
            $rutaGif = 'producto' . $id . '.' . $i . '.gif';
            $rutaPng = 'producto' . $id . '.' . $i . '.png';
