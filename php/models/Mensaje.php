...
class Mensaje extends Doctrine_Record {
	...
	public static function filtrarProducto ($cuantos, $desde, $productoId) {
		...
		$q = Doctrine_Query::create()
			...
			->innerJoin('m.estado s')
			->Where('s.id = 1')
			->andWhere('m.id_producto = '.$productoId)
			->limit($cuantos) //cuantos trae
			->offset($desde); //a partir de donde empieza a traer
	...
	public static function contarfiltrarProducto ($productoId) {
		$q = Doctrine_Query::create()
			->select('COUNT(m.id) as total')
			->from('Mensaje m')
			->innerJoin('m.emisor e')
