...
class Publicacion extends Doctrine_Record {
	...
	public function getTiempoRestante () {
		...
		if ($dias>=1) {
			return ($dias==1)?'�ltimo d�a':$dias.' d�as';
		} else {
			return 'publicaci�n vencida';
		}
	}
	...
	//listar es una funcion especial, porque junta en un mismo query la consulta de los productos y su paginado
	static public function listar ($parametros) {
		$defualt = array('categoria'=>'%', 'subcategoria'=>'%', 'precioMenor'=>0, 'precioMayor'=>1000000, 'periodicidad'=>'%', 'provincia'=>'%', 'localidad'=>'%', 'orderBy'=>'pb.inicio', 'sentido'=>'desc', 'cuantos'=>2, 'desde'=>0);
		...
		$q = Doctrine_Query::create()
			...
			->innerJoin('p.sugerencia sg')
			->innerJoin('p.periodicidad r')
			->innerJoin('p.direccion d')
			->innerJoin('d.localidad l')
			->where('c.id like ?', array($options['categoria']))
			->andWhere('pb.finalizacion >= "'.date('Y-m-d').'"')
			->andWhere('s.id like ?', array($options['subcategoria']))
			->andWhere('sg.precio >= '.$options['precioMenor'].' and sg.precio <= '.$options['precioMayor'])
	...
	static public function contar ($parametros) {
		$defualt = array('categoria'=>'%', 'subcategoria'=>'%', 'precioMenor'=>0, 'precioMayor'=>1000000, 'periodicidad'=>'%', 'provincia'=>'%', 'localidad'=>'%', 'orderBy'=>'pb.inicio', 'sentido'=>'desc');
		$options = $parametros + $defualt;
		$q = Doctrine_Query::create()
